...
author=
substitutions="-e s%@now@%${nowdate}%g -e s%@then@%${thendate}%g -e s%@author@%${author}%g"
...
sed ${substitutions} config.json.in > config.json
...
sed ${substitutions} manifest.json.in > manifest.json
